(function () {
  ...
  const text = {
    ...
    'No-auth GHCP harness · S2S app identity /3p (private preview diagnostic)': {
      'zh-CN': '无身份验证 GHCP 框架 · S2S 应用身份 /3p（专用预览诊断）',
      'zh-TW': '無驗證 GHCP 框架 · S2S 應用程式身分 /3p（私人預覽診斷）',
      ja: '認証なし GHCP ハーネス · S2S アプリ ID /3p（プライベート プレビュー診断）',
      ko: '인증 없는 GHCP 하네스 · S2S 앱 ID /3p(비공개 미리 보기 진단)'
    },
    'S2S application (client) ID': { 'zh-CN': 'S2S 应用程序（客户端）ID', 'zh-TW': 'S2S 應用程式（用戶端）ID', ja: 'S2S アプリケーション（クライアント）ID', ko: 'S2S 애플리케이션(클라이언트) ID' },
    'S2S directory (tenant) ID': { 'zh-CN': 'S2S 目录（租户）ID', 'zh-TW': 'S2S 目錄（租用戶）ID', ja: 'S2S ディレクトリ（テナント）ID', ko: 'S2S 디렉터리(테넌트) ID' },
    'S2S client secret': { 'zh-CN': 'S2S 客户端密码', 'zh-TW': 'S2S 用戶端密碼', ja: 'S2S クライアント シークレット', ko: 'S2S 클라이언트 비밀' },
    'Save S2S settings locally': { 'zh-CN': '在本地保存 S2S 设置', 'zh-TW': '在本機儲存 S2S 設定', ja: 'S2S 設定をローカルに保存', ko: 'S2S 설정을 로컬에 저장' },
    ...
    'Acquiring S2S app token…': { 'zh-CN': '正在获取 S2S 应用令牌…', 'zh-TW': '正在取得 S2S 應用程式權杖…', ja: 'S2S アプリ トークンを取得中…', ko: 'S2S 앱 토큰을 가져오는 중…' },
    'Online · S2S app identity connected': { 'zh-CN': '在线 · S2S 应用身份已连接', 'zh-TW': '線上 · S2S 應用程式身分已連線', ja: 'オンライン · S2S アプリ ID 接続済み', ko: '온라인 · S2S 앱 ID 연결됨' },
    'S2S settings saved locally and active. The secret is not returned to the browser.': { 'zh-CN': 'S2S 设置已在本地保存并启用。密码不会返回给浏览器。', 'zh-TW': 'S2S 設定已在本機儲存並啟用。密碼不會傳回瀏覽器。', ja: 'S2S 設定をローカルに保存して有効化しました。シークレットはブラウザーに返されません。', ko: 'S2S 설정이 로컬에 저장되고 활성화되었습니다. 비밀은 브라우저로 반환되지 않습니다.' },
    'S2S is not configured: {message}': { 'zh-CN': 'S2S 未配置：{message}', 'zh-TW': 'S2S 未設定：{message}', ja: 'S2S が構成されていません: {message}', ko: 'S2S가 구성되지 않았습니다: {message}' },
    ...
    // --- Slides 12-20 (new-agent runtimes, GHCP /3p, Agent Framework, S2S, takeaways) ---
    ...
    // --- Slides 17-19 (S2S app identity, live gate finding, sharing boundary) ---
    'S2S Private Preview': { 'zh-CN': 'S2S 专用预览', 'zh-TW': 'S2S 私人預覽', ja: 'S2S プライベート プレビュー', ko: 'S2S 비공개 미리 보기' },
    'S2S Private Preview · Integration Pattern': { 'zh-CN': 'S2S 专用预览 · 集成模式', 'zh-TW': 'S2S 私人預覽 · 整合模式', ja: 'S2S プライベート プレビュー · 統合パターン', ko: 'S2S 비공개 미리 보기 · 통합 패턴' },
    'App-only S2S adds a new identity branch to the guarded /3p route': { 'zh-CN': '仅应用 S2S 为受保护的 /3p 路由增加新的身份分支', 'zh-TW': '僅應用程式 S2S 為受保護的 /3p 路由增加新的身分分支', ja: 'アプリ専用 S2S が保護された /3p ルートに新しい ID 分岐を追加', ko: '앱 전용 S2S가 보호된 /3p 경로에 새 ID 분기 추가' },
    'The browser configures a confidential Entra application, but the Node sidecar owns the client secret, app-token cache, `/3p` preflight, conversation session, and SSE stream.': { 'zh-CN': '浏览器配置机密 Entra 应用，但客户端密码、应用令牌缓存、`/3p` 预检、对话会话和 SSE 流均由 Node 边车管理。', 'zh-TW': '瀏覽器設定機密 Entra 應用程式，但用戶端密碼、應用程式權杖快取、`/3p` 預檢、對話工作階段和 SSE 串流均由 Node Sidecar 管理。', ja: 'ブラウザーは機密 Entra アプリを構成しますが、クライアント シークレット、アプリ トークン キャッシュ、`/3p` 事前確認、会話セッション、SSE ストリームは Node サイドカーが管理します。', ko: '브라우저는 기밀 Entra 앱을 구성하지만 클라이언트 비밀, 앱 토큰 캐시, `/3p` 사전 점검, 대화 세션 및 SSE 스트림은 Node 사이드카가 관리합니다.' },
    'The browser configures a confidential Entra application, but the Node sidecar owns the client secret, app-token cache, /3p preflight, conversation session, and SSE stream.': { 'zh-CN': '浏览器配置机密 Entra 应用，但客户端密码、应用令牌缓存、/3p 预检、对话会话和 SSE 流均由 Node 边车管理。', 'zh-TW': '瀏覽器設定機密 Entra 應用程式，但用戶端密碼、應用程式權杖快取、/3p 預檢、對話工作階段和 SSE 串流均由 Node Sidecar 管理。', ja: 'ブラウザーは機密 Entra アプリを構成しますが、クライアント シークレット、アプリ トークン キャッシュ、/3p 事前確認、会話セッション、SSE ストリームは Node サイドカーが管理します。', ko: '브라우저는 기밀 Entra 앱을 구성하지만 클라이언트 비밀, 앱 토큰 캐시, /3p 사전 점검, 대화 세션 및 SSE 스트림은 Node 사이드카가 관리합니다.' },
    '1 · configure': { 'zh-CN': '1 · 配置', 'zh-TW': '1 · 設定', ja: '1 · 構成', ko: '1 · 구성' },
    'Enter client ID, tenant ID, secret, environment, and schema in the S2S mode.': { 'zh-CN': '在 S2S 模式中输入客户端 ID、租户 ID、密码、环境和架构名称。', 'zh-TW': '在 S2S 模式中輸入用戶端 ID、租用戶 ID、密碼、環境和結構描述名稱。', ja: 'S2S モードでクライアント ID、テナント ID、シークレット、環境、スキーマを入力します。', ko: 'S2S 모드에서 클라이언트 ID, 테넌트 ID, 비밀, 환경 및 스키마를 입력합니다.' },
    'Client ID, tenant, secret, environment, and schema.': { 'zh-CN': '客户端 ID、租户、密码、环境和架构。', 'zh-TW': '用戶端 ID、租用戶、密碼、環境和結構描述。', ja: 'クライアント ID、テナント、シークレット、環境、スキーマ。', ko: '클라이언트 ID, 테넌트, 비밀, 환경 및 스키마.' },
    'Save locally': { 'zh-CN': '在本地保存', 'zh-TW': '在本機儲存', ja: 'ローカル保存', ko: '로컬 저장' },
    '2 · acquire': { 'zh-CN': '2 · 获取', 'zh-TW': '2 · 取得', ja: '2 · 取得', ko: '2 · 획득' },
    'MSAL Node uses client credentials to mint an app-only Power Platform token.': { 'zh-CN': 'MSAL Node 使用客户端凭据生成仅应用的 Power Platform 令牌。', 'zh-TW': 'MSAL Node 使用用戶端認證產生僅應用程式的 Power Platform 權杖。', ja: 'MSAL Node がクライアント資格情報でアプリ専用 Power Platform トークンを発行します。', ko: 'MSAL Node가 클라이언트 자격 증명으로 앱 전용 Power Platform 토큰을 발급합니다.' },
    'MSAL Node mints an app-only Power Platform token.': { 'zh-CN': 'MSAL Node 生成仅应用的 Power Platform 令牌。', 'zh-TW': 'MSAL Node 產生僅應用程式的 Power Platform 權杖。', ja: 'MSAL Node がアプリ専用 Power Platform トークンを発行します。', ko: 'MSAL Node가 앱 전용 Power Platform 토큰을 발급합니다.' },
    '3 · guard': { 'zh-CN': '3 · 防护', 'zh-TW': '3 · 防護', ja: '3 · ガード', ko: '3 · 보호' },
    'Loopback endpoints validate the exact commercial environment host and `/3p` path.': { 'zh-CN': '回环终结点验证精确的商业环境主机和 `/3p` 路径。', 'zh-TW': '回環端點驗證精確的商業環境主機和 `/3p` 路徑。', ja: 'ループバック エンドポイントが商用環境ホストと `/3p` パスを厳密に検証します。', ko: '루프백 엔드포인트가 상용 환경 호스트와 `/3p` 경로를 정확히 검증합니다.' },
    'Loopback validates the commercial host and exact /3p path.': { 'zh-CN': '回环验证商业主机和精确的 /3p 路径。', 'zh-TW': '回環驗證商業主機和精確的 /3p 路徑。', ja: 'ループバックが商用ホストと正確な /3p パスを検証します。', ko: '루프백이 상용 호스트와 정확한 /3p 경로를 검증합니다.' },
    '4 · stream': { 'zh-CN': '4 · 流式传输', 'zh-TW': '4 · 串流', ja: '4 · ストリーム', ko: '4 · 스트림' },
    'CopilotStudioClient keeps the app-identity conversation and relays SSE as NDJSON.': { 'zh-CN': 'CopilotStudioClient 保持应用身份对话，并将 SSE 作为 NDJSON 中继。', 'zh-TW': 'CopilotStudioClient 維持應用程式身分對話，並將 SSE 以 NDJSON 轉送。', ja: 'CopilotStudioClient がアプリ ID 会話を維持し、SSE を NDJSON として中継します。', ko: 'CopilotStudioClient가 앱 ID 대화를 유지하고 SSE를 NDJSON으로 릴레이합니다.' },
    'The sidecar owns the conversation and relays SSE as NDJSON.': { 'zh-CN': '边车管理对话并将 SSE 作为 NDJSON 中继。', 'zh-TW': 'Sidecar 管理對話並將 SSE 以 NDJSON 轉送。', ja: 'サイドカーが会話を管理し、SSE を NDJSON として中継します。', ko: '사이드카가 대화를 관리하고 SSE를 NDJSON으로 릴레이합니다.' },
    'Credential boundary': { 'zh-CN': '凭据边界', 'zh-TW': '認證邊界', ja: '資格情報の境界', ko: '자격 증명 경계' },
    'config/s2s.local.json is atomically written, explicitly git-ignored, and requests owner-only permissions. The secret is submitted once to localhost, cleared from the form, never returned, and never stored in browser storage.': { 'zh-CN': 'config/s2s.local.json 以原子方式写入、明确被 Git 忽略，并请求仅所有者权限。密码仅提交到 localhost 一次，随后从表单清除，绝不返回，也不存入浏览器存储。', 'zh-TW': 'config/s2s.local.json 以原子方式寫入、明確由 Git 忽略，並要求僅擁有者權限。密碼只提交到 localhost 一次，之後從表單清除，絕不傳回，也不存入瀏覽器儲存體。', ja: 'config/s2s.local.json はアトミックに書き込まれ、明示的に Git 対象外となり、所有者専用権限を要求します。シークレットは localhost に一度だけ送信され、フォームから消去され、返却もブラウザー保存もされません。', ko: 'config/s2s.local.json은 원자적으로 기록되고 명시적으로 Git에서 제외되며 소유자 전용 권한을 요청합니다. 비밀은 localhost에 한 번만 전송되고 폼에서 지워지며 반환되거나 브라우저 저장소에 저장되지 않습니다.' },
    'Identity consistency': { 'zh-CN': '身份一致性', 'zh-TW': '身分一致性', ja: 'ID の一貫性', ko: 'ID 일관성' },
    'Preflight/start/send acquire the app token server-side. Sessions are tagged s2s, so delegated and app-only identities cannot continue each other\'s conversations. Blank-secret updates preserve the saved secret.': { 'zh-CN': '预检/启动/发送均在服务器端获取应用令牌。会话标记为 s2s，因此委托身份和仅应用身份无法继续彼此的对话。空密码更新会保留已保存密码。', 'zh-TW': '預檢/啟動/傳送均在伺服器端取得應用程式權杖。工作階段標記為 s2s，因此委派身分和僅應用程式身分無法繼續彼此的對話。空密碼更新會保留已儲存密碼。', ja: 'preflight/start/send はサーバー側でアプリ トークンを取得します。セッションは s2s としてタグ付けされるため、委任 ID とアプリ専用 ID は互いの会話を継続できません。空のシークレット更新では保存済みシークレットを保持します。', ko: 'preflight/start/send는 서버에서 앱 토큰을 획득합니다. 세션은 s2s로 태그되어 위임 ID와 앱 전용 ID가 서로의 대화를 계속할 수 없습니다. 빈 비밀 업데이트는 저장된 비밀을 유지합니다.' },
    'PATTERN — Treat app-only S2S as a separate authentication branch. Reuse the proven `/3p` URL guard and stream adapter, but never reuse delegated conversation state.': { 'zh-CN': '模式 — 将仅应用 S2S 视为独立的身份验证分支。复用已验证的 `/3p` URL 防护和流适配器，但绝不复用委托对话状态。', 'zh-TW': '模式 — 將僅應用程式 S2S 視為獨立的驗證分支。重複使用已驗證的 `/3p` URL 防護和串流配接器，但絕不重複使用委派對話狀態。', ja: 'パターン — アプリ専用 S2S を別の認証分岐として扱います。実証済みの `/3p` URL ガードとストリーム アダプターは再利用しても、委任会話状態は再利用しません。', ko: '패턴 — 앱 전용 S2S를 별도 인증 분기로 취급합니다. 검증된 `/3p` URL 보호와 스트림 어댑터는 재사용하되 위임 대화 상태는 재사용하지 않습니다.' },
    'PATTERN — Treat app-only S2S as a separate authentication branch. Reuse the proven /3p URL guard and stream adapter, but never reuse delegated conversation state.': { 'zh-CN': '模式 — 将仅应用 S2S 视为独立的身份验证分支。复用已验证的 /3p URL 防护和流适配器，但绝不复用委托对话状态。', 'zh-TW': '模式 — 將僅應用程式 S2S 視為獨立的驗證分支。重複使用已驗證的 /3p URL 防護和串流配接器，但絕不重複使用委派對話狀態。', ja: 'パターン — アプリ専用 S2S を別の認証分岐として扱います。実証済みの /3p URL ガードとストリーム アダプターは再利用しても、委任会話状態は再利用しません。', ko: '패턴 — 앱 전용 S2S를 별도 인증 분기로 취급합니다. 검증된 /3p URL 보호와 스트림 어댑터는 재사용하되 위임 대화 상태는 재사용하지 않습니다.' },
    'S2S Private Preview · Live Diagnostic': { 'zh-CN': 'S2S 专用预览 · 实时诊断', 'zh-TW': 'S2S 私人預覽 · 即時診斷', ja: 'S2S プライベート プレビュー · ライブ診断', ko: 'S2S 비공개 미리 보기 · 실시간 진단' },
    'Authentication passed; the platform feature gate stopped the call': { 'zh-CN': '身份验证已通过；平台功能门控阻止了调用', 'zh-TW': '驗證已通過；平台功能閘道阻止了呼叫', ja: '認証は成功、プラットフォーム機能ゲートで停止', ko: '인증은 통과했지만 플랫폼 기능 게이트가 호출 차단' },
    'Live-probed August 13, 2026 against the published no-auth agent mjsrc_mjnewagent4customer_Kvhq-e. A fresh client-credentials token reached the Agentic Runtime gateway.': { 'zh-CN': '2026 年 8 月 13 日对已发布的无身份验证智能体 mjsrc_mjnewagent4customer_Kvhq-e 进行实时探测。新获取的客户端凭据令牌已到达 Agentic Runtime 网关。', 'zh-TW': '2026 年 8 月 13 日對已發佈的無驗證代理程式 mjsrc_mjnewagent4customer_Kvhq-e 進行即時探測。新取得的用戶端認證權杖已到達 Agentic Runtime 閘道。', ja: '2026 年 8 月 13 日、公開済み認証なしエージェント mjsrc_mjnewagent4customer_Kvhq-e をライブ検証しました。新しいクライアント資格情報トークンは Agentic Runtime ゲートウェイに到達しました。', ko: '2026년 8월 13일 게시된 인증 없는 에이전트 mjsrc_mjnewagent4customer_Kvhq-e를 실시간 점검했습니다. 새 클라이언트 자격 증명 토큰이 Agentic Runtime 게이트웨이에 도달했습니다.' },
    'Live-probed August 13, 2026 against the published no-auth agent': { 'zh-CN': '2026 年 8 月 13 日对已发布的无身份验证智能体', 'zh-TW': '2026 年 8 月 13 日對已發佈的無驗證代理程式', ja: '2026 年 8 月 13 日、公開済み認証なしエージェント', ko: '2026년 8월 13일 게시된 인증 없는 에이전트' },
    '. A fresh client-credentials token reached the Agentic Runtime gateway.': { 'zh-CN': '进行了实时探测。新获取的客户端凭据令牌已到达 Agentic Runtime 网关。', 'zh-TW': '進行了即時探測。新取得的用戶端認證權杖已到達 Agentic Runtime 閘道。', ja: 'をライブ検証しました。新しいクライアント資格情報トークンは Agentic Runtime ゲートウェイに到達しました。', ko: '를 실시간 점검했습니다. 새 클라이언트 자격 증명 토큰이 Agentic Runtime 게이트웨이에 도달했습니다.' },
    'token audience': { 'zh-CN': '令牌受众', 'zh-TW': '權杖受眾', ja: 'トークン対象', ko: '토큰 대상' },
    'application context': { 'zh-CN': '应用上下文', 'zh-TW': '應用程式內容', ja: 'アプリケーション コンテキスト', ko: '애플리케이션 컨텍스트' },
    'application role present': { 'zh-CN': '应用角色存在', 'zh-TW': '應用程式角色存在', ja: 'アプリケーション ロールあり', ko: '애플리케이션 역할 있음' },
    'passed': { 'zh-CN': '已通过', 'zh-TW': '已通過', ja: '成功', ko: '통과' },
    'reached': { 'zh-CN': '已到达', 'zh-TW': '已到達', ja: '到達', ko: '도달' },
    'blocked': { 'zh-CN': '被阻止', 'zh-TW': '被阻止', ja: 'ブロック', ko: '차단' },
    'Client ID + secret authenticated in the same tenant.': { 'zh-CN': '客户端 ID + 密码已在同一租户中通过身份验证。', 'zh-TW': '用戶端 ID + 密碼已在同一租用戶中通過驗證。', ja: 'クライアント ID + シークレットが同一テナントで認証されました。', ko: '클라이언트 ID + 비밀이 동일 테넌트에서 인증되었습니다.' },
    'App credentials authenticated in the same tenant.': { 'zh-CN': '应用凭据已在同一租户中通过身份验证。', 'zh-TW': '應用程式認證已在同一租用戶中通過驗證。', ja: 'アプリ資格情報が同一テナントで認証されました。', ko: '앱 자격 증명이 동일 테넌트에서 인증되었습니다.' },
    'Power Platform application permission was present in the JWT roles claim.': { 'zh-CN': 'JWT roles 声明中存在 Power Platform 应用权限。', 'zh-TW': 'JWT roles 宣告中存在 Power Platform 應用程式權限。', ja: 'JWT roles クレームに Power Platform アプリケーション権限が存在しました。', ko: 'JWT roles 클레임에 Power Platform 애플리케이션 권한이 있었습니다.' },
    'JWT roles contains the Power Platform application permission.': { 'zh-CN': 'JWT roles 包含 Power Platform 应用权限。', 'zh-TW': 'JWT roles 包含 Power Platform 應用程式權限。', ja: 'JWT roles に Power Platform アプリケーション権限が含まれます。', ko: 'JWT roles에 Power Platform 애플리케이션 권한이 포함됩니다.' },
    'JWT roles contains the required application permission.': { 'zh-CN': 'JWT roles 包含所需的应用权限。', 'zh-TW': 'JWT roles 包含所需的應用程式權限。', ja: 'JWT roles に必要なアプリケーション権限が含まれます。', ko: 'JWT roles에 필요한 애플리케이션 권한이 포함됩니다.' },
    'Gateway routed the request to ThirdPartyAuthenticatedPublishedBotController.': { 'zh-CN': '网关将请求路由到 ThirdPartyAuthenticatedPublishedBotController。', 'zh-TW': '閘道將要求路由到 ThirdPartyAuthenticatedPublishedBotController。', ja: 'ゲートウェイが要求を ThirdPartyAuthenticatedPublishedBotController にルーティングしました。', ko: '게이트웨이가 요청을 ThirdPartyAuthenticatedPublishedBotController로 라우팅했습니다.' },
    'Gateway selected ThirdPartyAuthenticatedPublishedBotController.': { 'zh-CN': '网关选择了 ThirdPartyAuthenticatedPublishedBotController。', 'zh-TW': '閘道選擇了 ThirdPartyAuthenticatedPublishedBotController。', ja: 'ゲートウェイが ThirdPartyAuthenticatedPublishedBotController を選択しました。', ko: '게이트웨이가 ThirdPartyAuthenticatedPublishedBotController를 선택했습니다.' },
    'Gateway selected the third-party /3p controller.': { 'zh-CN': '网关选择了第三方 /3p 控制器。', 'zh-TW': '閘道選擇了第三方 /3p 控制器。', ja: 'ゲートウェイがサードパーティ /3p コントローラーを選択しました。', ko: '게이트웨이가 타사 /3p 컨트롤러를 선택했습니다.' },
    'Controller refused the request before agent ACL evaluation.': { 'zh-CN': '控制器在智能体 ACL 评估之前拒绝了请求。', 'zh-TW': '控制器在代理程式 ACL 評估之前拒絕了要求。', ja: 'コントローラーがエージェント ACL 評価前に要求を拒否しました。', ko: '컨트롤러가 에이전트 ACL 평가 전에 요청을 거부했습니다.' },
    'Controller stopped before agent ACL evaluation.': { 'zh-CN': '控制器在智能体 ACL 评估之前停止。', 'zh-TW': '控制器在代理程式 ACL 評估之前停止。', ja: 'コントローラーがエージェント ACL 評価前に停止しました。', ko: '컨트롤러가 에이전트 ACL 평가 전에 중지했습니다.' },
    'Feature gate stopped the call before agent ACL evaluation.': { 'zh-CN': '功能门控在智能体 ACL 评估之前停止了调用。', 'zh-TW': '功能閘道在代理程式 ACL 評估之前停止了呼叫。', ja: '機能ゲートがエージェント ACL 評価前に呼び出しを停止しました。', ko: '기능 게이트가 에이전트 ACL 평가 전에 호출을 중지했습니다.' },
    'Decisive service response': { 'zh-CN': '决定性服务响应', 'zh-TW': '決定性服務回應', ja: '決定的なサービス応答', ko: '결정적 서비스 응답' },
    'What this rules out': { 'zh-CN': '由此排除的原因', 'zh-TW': '由此排除的原因', ja: '除外できる原因', ko: '배제되는 원인' },
    'Not a bad secret, wrong audience, wrong tenant, missing application role, malformed environment host, or missing `/3p` controller route.': { 'zh-CN': '不是密码错误、受众错误、租户错误、缺少应用角色、环境主机格式错误或缺少 `/3p` 控制器路由。', 'zh-TW': '不是密碼錯誤、受眾錯誤、租用戶錯誤、缺少應用程式角色、環境主機格式錯誤或缺少 `/3p` 控制器路由。', ja: 'シークレット不良、対象不一致、テナント不一致、アプリ ロール不足、環境ホスト形式不良、`/3p` コントローラー不在ではありません。', ko: '잘못된 비밀, 대상, 테넌트, 누락된 앱 역할, 잘못된 환경 호스트 또는 `/3p` 컨트롤러 경로 누락이 아닙니다.' },
    'Secret, audience, tenant, app role, host shape, and /3p routing are all valid.': { 'zh-CN': '密码、受众、租户、应用角色、主机格式和 /3p 路由均有效。', 'zh-TW': '密碼、受眾、租用戶、應用程式角色、主機格式和 /3p 路由均有效。', ja: 'シークレット、対象、テナント、アプリ ロール、ホスト形式、/3p ルーティングはすべて有効です。', ko: '비밀, 대상, 테넌트, 앱 역할, 호스트 형식 및 /3p 라우팅이 모두 유효합니다.' },
    'What remains blocked': { 'zh-CN': '仍被阻止的部分', 'zh-TW': '仍被阻止的部分', ja: '残るブロック', ko: '남은 차단 요소' },
    'Microsoft must enable S2S D2E for the tenant/environment and confirm that the feature applies to GHCP harness `/3p`, not only standard/classic agents.': { 'zh-CN': 'Microsoft 必须为租户/环境启用 S2S D2E，并确认该功能适用于 GHCP 框架 `/3p`，而不仅是标准/经典智能体。', 'zh-TW': 'Microsoft 必須為租用戶/環境啟用 S2S D2E，並確認該功能適用於 GHCP 框架 `/3p`，而不僅是標準/傳統代理程式。', ja: 'Microsoft はテナント/環境で S2S D2E を有効化し、標準/クラシック エージェントだけでなく GHCP ハーネス `/3p` にも適用されることを確認する必要があります。', ko: 'Microsoft가 테넌트/환경에 S2S D2E를 활성화하고 표준/클래식 에이전트뿐 아니라 GHCP 하네스 `/3p`에도 적용됨을 확인해야 합니다.' },
    'Microsoft must enable S2S D2E and confirm GHCP /3p coverage.': { 'zh-CN': 'Microsoft 必须启用 S2S D2E 并确认 GHCP /3p 覆盖范围。', 'zh-TW': 'Microsoft 必須啟用 S2S D2E 並確認 GHCP /3p 涵蓋範圍。', ja: 'Microsoft は S2S D2E を有効化し、GHCP /3p の対象範囲を確認する必要があります。', ko: 'Microsoft가 S2S D2E를 활성화하고 GHCP /3p 지원 범위를 확인해야 합니다.' },
    'FINDING — A correctly formed app-only token is necessary but not sufficient. The server-side feature flag is evaluated before the agent-share ACL.': { 'zh-CN': '发现 — 正确构造的仅应用令牌是必要条件，但并不充分。服务器端功能标志在智能体共享 ACL 之前进行评估。', 'zh-TW': '發現 — 正確建構的僅應用程式權杖是必要條件，但並不充分。伺服器端功能旗標在代理程式共用 ACL 之前評估。', ja: '発見 — 正しく形成されたアプリ専用トークンは必要ですが十分ではありません。サーバー側機能フラグはエージェント共有 ACL より先に評価されます。', ko: '발견 — 올바른 앱 전용 토큰은 필요하지만 충분하지 않습니다. 서버 기능 플래그가 에이전트 공유 ACL보다 먼저 평가됩니다.' },
    'S2S Private Preview · Sharing Boundary': { 'zh-CN': 'S2S 专用预览 · 共享边界', 'zh-TW': 'S2S 私人預覽 · 共用邊界', ja: 'S2S プライベート プレビュー · 共有境界', ko: 'S2S 비공개 미리 보기 · 공유 경계' },
    'Why a no-auth GHCP agent may have no usable Share button': { 'zh-CN': '为何无身份验证 GHCP 智能体可能没有可用的共享按钮', 'zh-TW': '為何無驗證 GHCP 代理程式可能沒有可用的共用按鈕', ja: '認証なし GHCP エージェントに使える共有ボタンがない理由', ko: '인증 없는 GHCP 에이전트에 사용할 수 있는 공유 버튼이 없는 이유' },
    'The public GHCP sharing experience and the private-preview S2S ACL describe different identity models. Their prerequisites currently conflict in the new agent experience.': { 'zh-CN': '公开 GHCP 共享体验和专用预览 S2S ACL 描述了不同的身份模型。它们的先决条件目前在新智能体体验中相互冲突。', 'zh-TW': '公開 GHCP 共用體驗和私人預覽 S2S ACL 描述了不同的身分模型。它們的先決條件目前在新代理程式體驗中互相衝突。', ja: '公開 GHCP 共有エクスペリエンスとプライベート プレビュー S2S ACL は異なる ID モデルを記述しており、新しいエージェント エクスペリエンスでは前提条件が競合しています。', ko: '공개 GHCP 공유 환경과 비공개 미리 보기 S2S ACL은 서로 다른 ID 모델을 설명하며 새 에이전트 환경에서 전제 조건이 충돌합니다.' },
    'Public GHCP sharing': { 'zh-CN': '公开 GHCP 共享', 'zh-TW': '公開 GHCP 共用', ja: '公開 GHCP 共有', ko: '공개 GHCP 공유' },
    'Requires Authenticate with Microsoft.': { 'zh-CN': '要求“使用 Microsoft 进行身份验证”。', 'zh-TW': '要求「使用 Microsoft 進行驗證」。', ja: '「Microsoft で認証」が必要です。', ko: 'Microsoft로 인증이 필요합니다.' },
    'Shares with users, security groups, or the organization.': { 'zh-CN': '与用户、安全组或整个组织共享。', 'zh-TW': '與使用者、安全性群組或整個組織共用。', ja: 'ユーザー、セキュリティ グループ、組織と共有します。', ko: '사용자, 보안 그룹 또는 조직과 공유합니다.' },
    'No documented application-identity picker for a no-auth GHCP agent.': { 'zh-CN': '没有针对无身份验证 GHCP 智能体的已记录应用身份选择器。', 'zh-TW': '沒有針對無驗證 GHCP 代理程式的已記錄應用程式身分選擇器。', ja: '認証なし GHCP エージェント向けの文書化されたアプリ ID 選択機能はありません。', ko: '인증 없는 GHCP 에이전트용 문서화된 앱 ID 선택기가 없습니다.' },
    'Public channel matrix still marks Native app / Direct Line unavailable.': { 'zh-CN': '公开通道矩阵仍将原生应用 / Direct Line 标记为不可用。', 'zh-TW': '公開通道矩陣仍將原生應用程式 / Direct Line 標示為不可用。', ja: '公開チャネル表では Native app / Direct Line は引き続き利用不可です。', ko: '공개 채널 표에서 Native app / Direct Line은 여전히 사용할 수 없습니다.' },
    'Microsoft authentication is required.': { 'zh-CN': '需要 Microsoft 身份验证。', 'zh-TW': '需要 Microsoft 驗證。', ja: 'Microsoft 認証が必要です。', ko: 'Microsoft 인증이 필요합니다.' },
    'Targets users, security groups, or everyone.': { 'zh-CN': '面向用户、安全组或所有人。', 'zh-TW': '面向使用者、安全性群組或所有人。', ja: 'ユーザー、セキュリティ グループ、または全員が対象です。', ko: '사용자, 보안 그룹 또는 모든 사용자를 대상으로 합니다.' },
    'No app-identity picker is documented for no-auth GHCP.': { 'zh-CN': '没有为无身份验证 GHCP 记录应用身份选择器。', 'zh-TW': '沒有為無驗證 GHCP 記錄應用程式身分選擇器。', ja: '認証なし GHCP 向けのアプリ ID 選択機能は文書化されていません。', ko: '인증 없는 GHCP용 앱 ID 선택기가 문서화되지 않았습니다.' },
    'Native app / Direct Line remain unavailable.': { 'zh-CN': '原生应用 / Direct Line 仍不可用。', 'zh-TW': '原生應用程式 / Direct Line 仍不可用。', ja: 'Native app / Direct Line は引き続き利用できません。', ko: 'Native app / Direct Line은 계속 사용할 수 없습니다.' },
    'Private-preview S2S ACL': { 'zh-CN': '专用预览 S2S ACL', 'zh-TW': '私人預覽 S2S ACL', ja: 'プライベート プレビュー S2S ACL', ko: '비공개 미리 보기 S2S ACL' },
    'True app-only S2S requires No Authentication.': { 'zh-CN': '真正的仅应用 S2S 要求“无身份验证”。', 'zh-TW': '真正的僅應用程式 S2S 要求「無驗證」。', ja: '真のアプリ専用 S2S には「認証なし」が必要です。', ko: '진정한 앱 전용 S2S에는 인증 없음이 필요합니다.' },
    'Generic guide says share the agent with the app identity as Viewer.': { 'zh-CN': '通用指南要求将智能体以查看者身份共享给应用身份。', 'zh-TW': '通用指南要求將代理程式以檢視者身分共用給應用程式身分。', ja: '一般ガイドではアプリ ID を Viewer としてエージェントに共有します。', ko: '일반 가이드는 앱 ID를 Viewer로 에이전트에 공유하라고 합니다.' },
    'The guide does not document GHCP/new-experience `/3p` coverage.': { 'zh-CN': '该指南未记录 GHCP/新体验 `/3p` 的覆盖范围。', 'zh-TW': '該指南未記錄 GHCP/新體驗 `/3p` 的涵蓋範圍。', ja: 'ガイドは GHCP/新エクスペリエンス `/3p` の対象範囲を文書化していません。', ko: '가이드는 GHCP/새 환경 `/3p` 지원 범위를 문서화하지 않습니다.' },
    'No client-side workaround exists for feature-gate or app-share ACL rejection.': { 'zh-CN': '对于功能门控或应用共享 ACL 拒绝，不存在客户端解决方法。', 'zh-TW': '對於功能閘道或應用程式共用 ACL 拒絕，不存在用戶端解決方法。', ja: '機能ゲートまたはアプリ共有 ACL 拒否を回避するクライアント側手段はありません。', ko: '기능 게이트 또는 앱 공유 ACL 거부에 대한 클라이언트 우회 방법은 없습니다.' },
    'No Authentication is required.': { 'zh-CN': '需要“无身份验证”。', 'zh-TW': '需要「無驗證」。', ja: '「認証なし」が必要です。', ko: '인증 없음이 필요합니다.' },
    'Generic S2S guide expects app-identity Viewer access.': { 'zh-CN': '通用 S2S 指南要求应用身份具有查看者访问权限。', 'zh-TW': '通用 S2S 指南要求應用程式身分具有檢視者存取權。', ja: '一般 S2S ガイドはアプリ ID の Viewer アクセスを前提とします。', ko: '일반 S2S 가이드는 앱 ID Viewer 액세스를 전제로 합니다.' },
    'GHCP /3p support is not documented.': { 'zh-CN': 'GHCP /3p 支持尚未记录。', 'zh-TW': 'GHCP /3p 支援尚未記錄。', ja: 'GHCP /3p サポートは文書化されていません。', ko: 'GHCP /3p 지원이 문서화되지 않았습니다.' },
    'Feature and ACL failures cannot be bypassed by the client.': { 'zh-CN': '客户端无法绕过功能和 ACL 失败。', 'zh-TW': '用戶端無法繞過功能和 ACL 失敗。', ja: '機能および ACL エラーをクライアントから回避することはできません。', ko: '클라이언트가 기능 및 ACL 실패를 우회할 수 없습니다.' },
    'now': { 'zh-CN': '现在', 'zh-TW': '現在', ja: '現在', ko: '지금' },
    'then': { 'zh-CN': '然后', 'zh-TW': '然後', ja: '次に', ko: '다음' },
    'if denied': { 'zh-CN': '如果被拒绝', 'zh-TW': '如果被拒絕', ja: '拒否された場合', ko: '거부된 경우' },
    'Request Microsoft enablement for the environment.': { 'zh-CN': '请求 Microsoft 为环境启用该功能。', 'zh-TW': '要求 Microsoft 為環境啟用該功能。', ja: 'Microsoft に環境の有効化を依頼します。', ko: 'Microsoft에 환경 활성화를 요청합니다.' },
    'Retest and inspect the next authorization response.': { 'zh-CN': '重新测试并检查下一条授权响应。', 'zh-TW': '重新測試並檢查下一個授權回應。', ja: '再テストして次の認可応答を確認します。', ko: '다시 테스트하고 다음 권한 부여 응답을 확인합니다.' },
    'Ask Microsoft for the GHCP app-identity Share API or ACL provisioning path.': { 'zh-CN': '向 Microsoft 请求 GHCP 应用身份共享 API 或 ACL 预配路径。', 'zh-TW': '向 Microsoft 要求 GHCP 應用程式身分共用 API 或 ACL 佈建路徑。', ja: 'Microsoft に GHCP アプリ ID 共有 API または ACL プロビジョニング経路を確認します。', ko: 'Microsoft에 GHCP 앱 ID 공유 API 또는 ACL 프로비저닝 경로를 요청합니다.' },
    'Request environment enablement.': { 'zh-CN': '请求启用环境功能。', 'zh-TW': '要求啟用環境功能。', ja: '環境の有効化を依頼します。', ko: '환경 활성화를 요청합니다.' },
    'Retest the next authorization stage.': { 'zh-CN': '重新测试下一授权阶段。', 'zh-TW': '重新測試下一個授權階段。', ja: '次の認可段階を再テストします。', ko: '다음 권한 부여 단계를 다시 테스트합니다.' },
    'Request the app-share API or ACL path.': { 'zh-CN': '请求应用共享 API 或 ACL 路径。', 'zh-TW': '要求應用程式共用 API 或 ACL 路徑。', ja: 'アプリ共有 API または ACL 経路を依頼します。', ko: '앱 공유 API 또는 ACL 경로를 요청합니다.' },
    'SUPPORT BOUNDARY — Do not switch the agent to Microsoft authentication just to reveal Share; that invalidates true app-only S2S. Enablement and app ACL must come from the preview service.': { 'zh-CN': '支持边界 — 不要仅为了显示共享按钮而将智能体切换为 Microsoft 身份验证；这会使真正的仅应用 S2S 失效。功能启用和应用 ACL 必须由预览服务提供。', 'zh-TW': '支援界線 — 不要只為了顯示共用按鈕而將代理程式切換為 Microsoft 驗證；這會使真正的僅應用程式 S2S 失效。功能啟用和應用程式 ACL 必須由預覽服務提供。', ja: 'サポート境界 — 共有を表示するためだけに Microsoft 認証へ切り替えないでください。真のアプリ専用 S2S が無効になります。有効化とアプリ ACL はプレビュー サービス側で必要です。', ko: '지원 경계 — 공유를 표시하려고 에이전트를 Microsoft 인증으로 바꾸지 마세요. 진정한 앱 전용 S2S가 무효화됩니다. 활성화와 앱 ACL은 미리 보기 서비스에서 제공되어야 합니다.' },
    'SUPPORT BOUNDARY — Do not switch to Microsoft authentication just to expose Share; it breaks true app-only S2S. Preview enablement and the app ACL must be provisioned server-side.': { 'zh-CN': '支持边界 — 不要仅为了显示共享而切换到 Microsoft 身份验证；这会破坏真正的仅应用 S2S。预览启用和应用 ACL 必须在服务器端预配。', 'zh-TW': '支援界線 — 不要只為了顯示共用而切換到 Microsoft 驗證；這會破壞真正的僅應用程式 S2S。預覽啟用和應用程式 ACL 必須在伺服器端佈建。', ja: 'サポート境界 — 共有を表示するためだけに Microsoft 認証へ切り替えないでください。真のアプリ専用 S2S が壊れます。プレビュー有効化とアプリ ACL はサーバー側でプロビジョニングする必要があります。', ko: '지원 경계 — 공유를 표시하려고 Microsoft 인증으로 전환하지 마세요. 진정한 앱 전용 S2S가 깨집니다. 미리 보기 활성화와 앱 ACL은 서버에서 프로비저닝되어야 합니다.' },
    'If you remember five things': { 'zh-CN': '请记住五件事', 'zh-TW': '請記住五件事', ja: '覚えておくべき 5 つのこと', ko: '기억해야 할 다섯 가지' },
    'App-only S2S is private preview: correct tokens still require EnableS2SAuthFeature and an app-identity ACL.': { 'zh-CN': '仅应用 S2S 处于专用预览：正确的令牌仍需要 EnableS2SAuthFeature 和应用身份 ACL。', 'zh-TW': '僅應用程式 S2S 處於私人預覽：正確的權杖仍需要 EnableS2SAuthFeature 和應用程式身分 ACL。', ja: 'アプリ専用 S2S はプライベート プレビューです。正しいトークンでも EnableS2SAuthFeature とアプリ ID ACL が必要です。', ko: '앱 전용 S2S는 비공개 미리 보기입니다. 올바른 토큰에도 EnableS2SAuthFeature와 앱 ID ACL이 필요합니다.' },
    'App-only S2S is private preview: correct tokens still require': { 'zh-CN': '仅应用 S2S 处于专用预览：正确的令牌仍需要', 'zh-TW': '僅應用程式 S2S 處於私人預覽：正確的權杖仍需要', ja: 'アプリ専用 S2S はプライベート プレビューです。正しいトークンでも', ko: '앱 전용 S2S는 비공개 미리 보기입니다. 올바른 토큰에도' },
    'and an app-identity ACL.': { 'zh-CN': '和应用身份 ACL。', 'zh-TW': '和應用程式身分 ACL。', ja: 'とアプリ ID ACL が必要です。', ko: '와 앱 ID ACL이 필요합니다.' },
    '16 · S2S ARCHITECTURE': { 'zh-CN': '16 · S2S 架构', 'zh-TW': '16 · S2S 架構', ja: '16 · S2S アーキテクチャ', ko: '16 · S2S 아키텍처' },
    '17 · S2S LIVE DIAGNOSTIC': { 'zh-CN': '17 · S2S 实时诊断', 'zh-TW': '17 · S2S 即時診斷', ja: '17 · S2S ライブ診断', ko: '17 · S2S 실시간 진단' },
    '18 · S2S SHARING BOUNDARY': { 'zh-CN': '18 · S2S 共享边界', 'zh-TW': '18 · S2S 共用邊界', ja: '18 · S2S 共有境界', ko: '18 · S2S 공유 경계' },
    '19 · TAKEAWAYS': { 'zh-CN': '19 · 要点', 'zh-TW': '19 · 重點', ja: '19 · 要点', ko: '19 · 핵심 요점' },
    '20 · COMMUNITY SHARE': { 'zh-CN': '20 · 社区分享', 'zh-TW': '20 · 社區分享', ja: '20 · コミュニティ共有', ko: '20 · 커뮤니티 공유' },
